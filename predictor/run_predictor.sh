...
if ! docker image inspect consumo-predictor:latest >/dev/null 2>&1; then
    echo "❌ Error: La imagen consumo-predictor:latest no existe"
...
echo "✅ Imagen construida exitosamente: consumo-predictor:latest"ker run --rm -e SEPAL_LENGTH=6.0 -e SEPAL_WIDTH=3.0 -e PETAL_LENGTH=4.5 -e PETAL_WIDTH=1.5 iris-predictor:latest python predict_api.py --env"
echo ""
echo "🌟 Salida en JSON:"
